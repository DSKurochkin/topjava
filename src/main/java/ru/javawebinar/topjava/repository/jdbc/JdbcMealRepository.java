...
public class JdbcMealRepository implements MealRepository {
    ...
    public Meal save(Meal meal, int userId) {
        ...
        if (meal.isNew()) {
            ...
            meal.setId(newKey.intValue());
                ...
                "UPDATE meals SET datetime=?, description=?" +
                        ", calories=? WHERE id=? and user_id=?", meal.getDateTime(), meal.getDescription()
                , meal.getCalories(), meal.getId(), userId) == 0) {
            return null;
        }
        return meal;
    }
    ...
    public List<Meal> getBetweenHalfOpen(LocalDateTime startDateTime, LocalDateTime endDateTime, int userId) {
        return jdbcTemplate.query("SELECT * FROM meals WHERE datetime>=? AND datetime<=? AND user_id=?"
                , ROW_MAPPER, startDateTime.toLocalDate(), endDateTime.toLocalDate(), userId);
    }
}
